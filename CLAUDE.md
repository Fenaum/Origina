...
**Program plan:** [docs/sprints/MILESTONES.md](docs/sprints/MILESTONES.md) — a milestone every 5 sprints, closed by owner UAT. **M1 Pilot-Ready** = Sprints 1–5 ✅ (UAT-1 in progress). **M2 Operational Depth** = Sprints 6–10 (docs/URLA/processing/funding/decisioning — "start-to-fund without leaving Origina"), implementable specs written. **M3 Enterprise-Ready** = Sprints 11–15 (org hierarchy, SLA engine, SSO, SOC 2 groundwork, read-only API preview), draft specs to be promoted at the M2 gate. **M4 Platform** = Sprints 16–20 (external API GA, event platform + async jobs, guarded write API, SDK + first partner integration, SLOs), outline pre-draft specs — entry criteria and pivot triggers in MILESTONES.md, promoted at the M3 gate.

**Sprint 6 — Closeout & UAT Response — ✅ closed 2026-07-11.** Phase 6.0 cleared the four Sprint 1–5 audit debts (BUG-2026-07-11-001 fix + intake-lifecycle test un-skipped + new multi-tenant regression; `tests/backend/test_rbac_coverage.py` role × route matrix; `tests/frontend/BugRegressions.test.tsx` for BUG-001/002/003 — BUG-002 surfaced via the new test and was fixed in-sprint, logged as BUG-2026-07-11-002). Phase 6.0.5 (migration-based test runner) **slipped** — production migrations need schema qualification before they can be replayed into a per-test schema; destination is the first schema-debt-focused sprint (flagged in the Sprint 6 archive's *What slipped*). Phase 6.1 (UAT-1 burn-down) is owner-driven and remains open — no P0/P1 had landed by sprint close. Phase 6.2 added `GET /loans/{id}/detail` (replaces the `limit=1000` pipeline hack), split Borrower/Property into dedicated `BorrowerPanel` + `PropertyPanel` (read-only this sprint), and switched `useLoanDetail` to the new endpoint; 6 new tests across both. Spec: [docs/sprints/sprint-6-build-spec.md](docs/sprints/sprint-6-build-spec.md). Archive: [docs/sprints/sprint-6-closeout-uat-response.md](docs/sprints/sprint-6-closeout-uat-response.md).

**Sprint 7 — Document Platform v1 — In progress (started 2026-07-16):** real S3-compatible object storage behind a `StorageBackend` protocol (MinIO in dev/CI, AWS/R2/B2 in prod — env-var swap, not code change), document→condition auto-linking via a tenant-overridable category map, document versioning chain, plus a borrower-facing condition view endpoint. ADR-first: write the storage-provider ADR in DECISIONS.md before coding. Sprint 7 is the #1 prerequisite for the future AI layer (per AI Scope ADR). Spec: [docs/sprints/sprint-7-build-spec.md](docs/sprints/sprint-7-build-spec.md). Phases: 7.1 storage interface, 7.2 auto-link, 7.3 versioning + borrower view.
...
| ~~Role vocabulary split (frontend role names ≠ backend role constants)~~ | ~~High — RBAC correctness~~ | ~~Low — Sprint 2~~ | ✅ Done in Sprint 2 |
...
| ~~Test schema doesn't install triggers (`create_all()` skips PL/pgSQL)~~ | ~~Low — test infra friction~~ | ✅ Partial — Sprint 3 inline-install stopgap. Migration-based runner **slipped from Sprint 6 Phase 6.0** (production migrations need schema qualification first). Tracked in `docs/sprints/sprint-6-closeout-uat-response.md` *What slipped*; destined for the first schema-debt-focused sprint. |
| ~~RBAC route coverage — `test_rbac_coverage.py` role × route matrix never written; only `/users/` has RBAC tests~~ | ~~Medium — authz regressions invisible~~ | ✅ Done in Sprint 6 — `tests/backend/test_rbac_coverage.py` walks a representative slice of role × route; more coverage cells needed over time, baseline in place |
...
| ~~Frontend bugs BUG-2026-07-09-001/002/003 have manual-only regression coverage~~ | ~~Low — regressions could return silently~~ | ✅ Done in Sprint 6 — `tests/frontend/BugRegressions.test.tsx` covers all three. BUG-2026-07-11-002 also logged + covered. |
| ~~Skipped intake test masks a real error (`test_coverage_gaps.py:1005` — "Multiple rows were found")~~ | ~~Medium — possible product bug in intake flow~~ | ✅ Done in Sprint 6 — root cause was BUG-2026-07-11-001 (`_platform_tenant_id` `MultipleResultsFound`); blanket `try/except` removed, intake-lifecycle test un-skipped, new multi-tenant regression test added |
